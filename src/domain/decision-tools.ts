...
export function riskEMV(rows:{probability:number;impact:number;residualProbability:number;residualImpact:number;responseCost?:number}[]){
 if(rows.some(r=>!finite([r.probability,r.impact,r.residualProbability,r.residualImpact])||(r.responseCost!==undefined&&!Number.isFinite(r.responseCost))||r.probability<0||r.probability>100||r.residualProbability<0||r.residualProbability>100||r.impact<0||r.residualImpact<0||(r.responseCost??0)<0))throw Error('Invalid risk input');
 const risks=rows.map(r=>{const emv=r.probability/100*r.impact,residual=r.residualProbability/100*r.residualImpact,reduction=emv-residual,responseCost=r.responseCost??0;return {...r,emv,residual,reduction,responseCost,roi:responseCost>0?(reduction-responseCost)/responseCost:null};});
 return {risks,total:risks.reduce((s,r)=>s+r.emv,0),residual:risks.reduce((s,r)=>s+r.residual,0),responseCost:risks.reduce((s,r)=>s+r.responseCost,0),expectedReduction:risks.reduce((s,r)=>s+r.reduction,0),allOccur:rows.reduce((s,r)=>s+r.impact,0)};
}
...
export function estimateCalibration(w:Workspace,projectId:string){
 ...
 if(sample.length<10)return {count:sample.length,minimum:10,medianRatio:null,medianAbsoluteError:null,tendency:null,ratios:[] as number[]};
 ...
 const ratio=median(ratios);return {count:sample.length,minimum:10,medianRatio:ratio,medianAbsoluteError:median(errors),tendency:ratio>1.1?'underestimate':ratio<.9?'overestimate':'balanced',ratios};
}
export function ownershipCoverage(w:Workspace,projectId:string){
 ...
 const issues:{id:string;kind:string;title:string;reason:'unassigned'|'external'|'ambiguous'|'concentration'|'overloaded'}[]=[];
 ...
 w.milestones.filter(x=>x.projectId===projectId&&!['done','cancelled'].includes(x.status)).forEach(x=>inspect(x.id,'milestone',x.title,x.owner));
 const critical=active.filter(x=>x.priority==='critical');for(const m of members){const owns=(x:typeof active[number])=>x.ownerId===m.id||(!x.ownerId&&x.owner===m.name),count=critical.filter(owns).length,activeCount=active.filter(owns).length;if(critical.length>=3&&count/critical.length>.5)issues.push({id:m.id,kind:'team',title:m.name,reason:'concentration'});if(activeCount>=8)issues.push({id:m.id,kind:'team',title:m.name,reason:'overloaded'});}
 return issues;
}
