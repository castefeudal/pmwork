...
it('migrates v4 without changing ownership, views or locale and round trips context',()=>{const original=demoWorkspace('ru');original.projectSettings[0].context=defaultContext;const migrated=migrateWorkspace({...original,schemaVersion:4});expect(migrated.schemaVersion).toBe(5);for(const key of ['projects','workItems','savedWorkViews','locale','projectSettings'] as const)expect(migrated[key]).toEqual(original[key]);expect(migrateWorkspace(JSON.parse(JSON.stringify(migrated)))).toEqual(migrated)});
