...
describe("PM calculations", () => {
  it("calculates CPM and float", () => {
    const r = calculateCPM([
      { id: "A", duration: 3, predecessors: [] },
      { id: "B", duration: 4, predecessors: ["A"] },
      { id: "C", duration: 2, predecessors: ["A"] },
    ]);
    expect(r.find((x) => x.id === "B")?.critical).toBe(true);
    expect(r.find((x) => x.id === "C")?.float).toBe(2);
  });
  it("rejects cycles", () =>
    expect(() =>
      calculateCPM([
  ...
  it("calculates PERT", () =>
    expect(calculatePERT(2, 5, 8)).toEqual({
      expected: 5,
      standardDeviation: 1,
      variance: 1,
    }));
  ...
  it("calculates prioritization and flow", () => {
    expect(rice(100, 2, 80, 4)).toBe(40);
    expect(wsjf(5, 3, 2, 2)).toBe(5);
    expect(littleLaw(10, 2).cycleTime).toBe(5);
  });
  it("produces deterministic forecast percentiles", () => {
    const r = monteCarlo([1, 2, 3], 5, 100, () => 0.5);
    expect(r.p50).toBe(10);
    expect(r.p95).toBe(10);
  });
});
describe("method fit", () => {
  it("prefers adaptive for volatile autonomous work", () => {
    const r = scoreApproaches({
      uncertainty: 5,
      volatility: 5,
      feedback: 5,
      frequency: 5,
    ...
    expect(r[0]?.approach).toBe("Adaptive");
  });
  it("raises governance when needed", () =>
    expect(
      governanceLevel({
        uncertainty: 2,
        volatility: 2,
