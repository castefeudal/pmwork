...
export const glossaryTerms: GlossaryTerm[] = glossary.map(entry => {
 ...
 const context = contexts[category];
 ...
 whyItMatters: {ru:context[3],en:context[4]}, whenUsed:{ru:context[1],en:context[2]},
 related: [...new Set([...(confused[entry.term] ?? []),...entry.related])].filter(t=>glossary.some(g=>g.term===t)).map(slug),
 confusedWith:(confused[entry.term] ?? []).map(slug), tags:[category], workspaceLinks:[context[0]],methodLinks:[],templateLinks:[],toolLinks:[],sourceIds:[],reviewedAt:'2026-09-05' };
});
