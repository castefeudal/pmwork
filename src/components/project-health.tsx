...
export function ProjectHealth({workspace,project,locale,onView}: Pick<ViewProps,"workspace"|"project"|"locale"|"onView">) {
  ...
  const rows: {key:keyof typeof labels; bad:boolean; unknown?:boolean; why:string; view:Parameters<typeof onView>[0]}[] = [
    {key:"schedule",bad:overdue+conflicts>0,why:ru?`${overdue} просрочено · ${conflicts} конфликтов связей`:`${overdue} overdue · ${conflicts} dependency conflicts`,view:"planning"},
    ...
    {key:"delivery",bad:blocked>0,why:ru?`${blocked} блокеров · ${open.length} незавершённых`:`${blocked} blockers · ${open.length} open items`,view:"board"},
    {key:"risk",bad:risks>0,why:ru?`${risks} открытых рисков с P × I ≥ 15`:`${risks} open risks with P × I ≥ 15`,view:"raid"},
    ...
    {key:"team",bad:unowned>0,why:ru?`${unowned} без владельца. Это не оценка загрузки.`:`${unowned} unassigned. This is not a capacity estimate.`,view:"people"},
