...
export function GlossaryBrowser({terms,locale}: {terms:GlossaryTerm[];locale:Locale}) {
 ...
 return <section className="public-container glossary-browser">
  ...
  <label className="glossary-search">{ru?'Найти термин':'Find a term'}<input className="input" type="search" placeholder="WBS, ИСР, critical path…" value={query} onChange={e=>{setQuery(e.target.value);setLetter('');setVisible(24);}}/></label>
  <div className="glossary-layout"><aside className="glossary-categories" aria-label={ru?'Категории':'Categories'}><button className="button" aria-pressed={category==='all'} onClick={()=>{setCategory('all');setVisible(24);}}>{ru?'Все категории':'All categories'}</button>{glossaryCategories.map(c=><button className="button" key={c[0]} aria-pressed={category===c[0]} onClick={()=>{setCategory(c[0]);setVisible(24);}}>{c[ru?1:2]}</button>)}</aside>
   <div><label>{ru?'Уровень применения':'Guidance level'}<select className="input" value={level} onChange={e=>{setLevel(e.target.value);setVisible(24);}}>{[['all','Все','All'],['foundation','Основы','Foundation'],['practitioner','Практик','Practitioner'],['advanced','Продвинутый','Advanced']].map(l=><option value={l[0]} key={l[0]}>{l[ru?1:2]}</option>)}</select></label>
    <nav className="alphabet" aria-label="A–Z"><button aria-pressed={!letter} onClick={()=>{setLetter('');setVisible(24);}}>{ru?'Все':'All'}</button>{'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(l=><button key={l} aria-pressed={letter===l} onClick={()=>{setLetter(l);setVisible(24);}}>{l}</button>)}</nav>
    <p role="status">{ru?'Найдено':'Results'}: {results.length}</p>
    <div className="glossary-results">{results.slice(0,visible).map(t=><button className="glossary-row" key={t.slug} onClick={()=>setSelected(t)} aria-haspopup="dialog"><strong>{t.term}</strong><span>{t.ruTerm}</span><span>{t.plainLanguage[locale]}</span>{t.acronym&&<small>{t.aliases.find(a=>/[a-z]/i.test(a)&&a.includes(' '))}</small>}<small>{glossaryCategories.find(c=>c[0]===t.category)?.[ru?1:2]} · {ru?({foundation:"Основы",practitioner:"Практик",advanced:"Продвинутый"})[t.level]:t.level}</small></button>)}</div>
    {results.length>visible&&<button className="button" onClick={()=>setVisible(v=>v+24)}>{ru?"Показать ещё 24":"Show 24 more"}</button>}
