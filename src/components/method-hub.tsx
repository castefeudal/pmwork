...
export function MethodHub({locale,methods}:{locale:Locale;methods:Pick<Method,"slug"|"version"|"title"|"summary"|"bestFit"|"limitations">[]}){
 ...
 return <>
  <header className="catalog-hero"><p className="eyebrow">PMWORK · {ru?'методы':'methods'}</p><h1>{ru?'Библиотека методов · Выберите способ работы по контексту':'Methods library · Choose an approach by context'}</h1><p className="lead">{ru?'Метод — не знак зрелости. Сравнивайте его допущения с изменчивостью требований, сроками, зависимостями и доступностью обратной связи.':'A method is not a maturity badge. Compare its assumptions with volatility, deadlines, dependencies and feedback access.'}</p><div className="catalog-controls"><label className="field"><span className="sr-only">{ru?'Поиск':'Search'}</span><span className="search-input"><Search size={18}/><input className="input" value={query} onChange={e=>setQuery(e.target.value)} placeholder={ru?'Например: Scrum, Kanban, жёсткий срок…':'For example: Scrum, Kanban, fixed deadline…'}/></span></label><span className="pill">{data.length} {ru?'методов':'methods'}</span></div></header>
