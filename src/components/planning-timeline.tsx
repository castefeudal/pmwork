...
export function PlanningTimeline({workspace,project,locale,onEdit,onCreate}: Pick<ViewProps,"workspace"|"project"|"locale"|"onEdit"|"onCreate">) {
  const ru=locale==="ru", [zoom,setZoom]=useState("project"), [full,setFull]=useState(false);
  ...
  return <section className="panel planning-panel">
    ...
    <div className="mobile-planning-agenda"><h3>{ru ? "Ближайшая работа и контрольные точки" : "Upcoming work and milestones"}</h3>{[...scheduled.map(x=>({id:x.id,title:x.title,date:x.dueDate||x.startDate!,kind:"work" as const})),...milestones.map(x=>({id:x.id,title:x.title,date:x.date,kind:"milestone" as const}))].sort((a,b)=>a.date.localeCompare(b.date)).map(x=><button className="mobile-work-card" key={x.id} onClick={()=>onEdit(x.kind,x.id)}><small>{formatDate(x.date,locale)}</small><strong>{x.title}</strong></button>)}<button className="button" onClick={()=>setFull(!full)}>{full?(ru?"Скрыть timeline":"Hide timeline"):(ru?"Открыть полный timeline":"Open full timeline")}</button></div>
    {scheduled.length?<div className={`timeline-scroll ${full ? "mobile-expanded" : ""}`}><div className="date-timeline" style={{minWidth:width}}><div className="timeline-dates"><span>{formatDate(new Date(start).toISOString(),locale)}</span><span>{formatDate(new Date(end).toISOString(),locale)}</span></div>
      {scheduled.map(x=><div className="date-timeline-row" key={x.id}><button className="work-title-button" onClick={()=>onEdit("work",x.id)}><small>{x.id}</small> {x.title}<span className="timeline-date-label">{formatDate(x.startDate,locale)} → {formatDate(x.dueDate,locale)}</span></button><div className="date-track">{Date.parse(today)>=start&&Date.parse(today)<=end&&<span className="today-marker" style={{left:`${marker}%`}} title={ru?"Сегодня":"Today"}/>}<button className={`date-bar ${x.blocked||conflicts.some(c=>c.to===x.id)?"conflict":""}`} style={{left:`${position(x.startDate||x.dueDate!)}%`,width:`${Math.max(.6,position(x.dueDate||x.startDate!)-position(x.startDate||x.dueDate!)+100/days)}%`}} onClick={()=>onEdit("work",x.id)} aria-label={`${x.title}: ${x.startDate||"—"} → ${x.dueDate||"—"}`}>{x.blocked?(ru?"Блокер":"Blocked"):x.id}</button></div></div>)}
      {milestones.map(m=><div className="date-timeline-row" key={m.id}><button className="work-title-button" onClick={()=>onEdit("milestone",m.id)}>{m.title}<span className="timeline-date-label">{formatDate(m.date,locale)} · {m.progress}%</span></button><div className="date-track"><button className="milestone-marker" style={{left:`${position(m.date)}%`}} aria-label={`${m.title} ${m.date}`} onClick={()=>onEdit("milestone",m.id)}>◆</button></div></div>)}
