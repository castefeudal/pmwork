...
export function TodayView({workspace,project,locale,onView,onEdit,onCreate}:ViewProps){
 ...
 return <>
  ...
  <div className="today-layout"><div>{groups.map(group=><section className="panel" key={group.id}><h3>{group.title} <span className="muted">{group.items.length}</span></h3>{group.items.length?<ol className="action-list">{group.items.map(signal=>{const ref=references[signal.id];return <li key={signal.id}><span className={`signal-rank ${signal.severity}`}>{signal.severity==='critical'?(ru?'Критично':'Critical'):signal.severity==='high'?(ru?'Высокий':'High'):(ru?'Проверка':'Check')}</span><div><strong>{signal.title}</strong><p>{signal.why}</p>{ref&&<small>{ru?'Владелец':'Owner'}: {ref.owner||(ru?'не указан':'unassigned')}</small>}<p>{signal.action}</p></div><button className="button small" onClick={()=>openSignal(signal)}>{actionLabel(ru,signal.id,Boolean(ref))}</button></li>})}</ol>:<p className="muted">{ru?'Других сигналов в этой группе нет.':'No other signals in this group.'}</p>}</section>)}</div><aside><ProjectHealth workspace={workspace} project={project} locale={locale} onView={onView}/></aside></div>
 </>;
}
