...
export function TodayView({workspace,project,locale,onView,onEdit,onCreate}:ViewProps){
 ...
 const top=signals[0],topRef=top?references[top.id]:undefined,remaining=top?signals.filter(signal=>signal.id!==top.id):signals;
 const groups=[{id:'decision',title:ru?'Требуется решение':'Requires a decision',items:remaining.filter(s=>s.id.startsWith('decision-')||s.id.startsWith('change-'))},{id:'action',title:ru?'Требуется действие':'Requires action',items:remaining.filter(s=>!s.id.startsWith('decision-')&&!s.id.startsWith('change-')&&s.severity!=='medium')},{id:'check',title:ru?'Стоит проверить':'Worth checking',items:remaining.filter(s=>s.severity==='medium')}];
 ...
 return <>
  ...
  <div className="today-layout"><div>{groups.map(group=><section className="panel" key={group.id}><h3>{group.title} <span className="muted">{group.items.length}</span></h3>{group.items.length?<ol className="action-list">{group.items.map(signal=>{const ref=references[signal.id];return <li key={signal.id}><span className={`signal-rank ${signal.severity}`}>{signal.severity==='critical'?(ru?'Критично':'Critical'):signal.severity==='high'?(ru?'Высокий':'High'):(ru?'Проверка':'Check')}</span><div><strong>{signal.title}</strong><p>{signal.why}</p>{ref&&<small>{ru?'Владелец':'Owner'}: {ref.owner||(ru?'не указан':'unassigned')}</small>}<p>{signal.action}</p></div><button className="button small" onClick={()=>openSignal(signal)}>{actionLabel(ru,signal.id,Boolean(ref))}</button></li>})}</ol>:<p className="muted">{ru?'Других сигналов в этой группе нет.':'No other signals in this group.'}</p>}</section>)}<section className="panel"><h3>{ru?'Стабильно':'Stable'} <span className="muted">{stable.length}</span></h3>{stable.length?<ul>{stable.map(item=><li key={item}>{item}</li>)}</ul>:<p className="muted">{ru?'Недостаточно текущих данных, чтобы подтвердить стабильные области. Отсутствие записи не считается зелёным сигналом.':'There is not enough current data to confirm a stable area. Missing records are not treated as green.'}</p>}</section></div><aside><ProjectHealth workspace={workspace} project={project} locale={locale} onView={onView}/></aside></div>
 </>;
}
