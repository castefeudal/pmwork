...
export function TodayView({workspace,project,locale,onView,onEdit,onCreate}:ViewProps){
 ...
 const groups=categories.map(([id,title])=>({id,title,items:remaining.filter(s=>s.category===id)})).filter(group=>group.items.length);
 ...
 return <>
  ...
  <div className="today-layout"><div>{groups.map(group=><section className="panel" key={group.id}><h3>{group.title} <span className="muted">{group.items.length}</span></h3>{group.items.length?<ol className="action-list">{(expanded.includes(group.id)?group.items:group.items.slice(0,5)).map(signal=>{const ref=signal.source;return <li key={signal.id}><span className={`signal-rank ${signal.severity}`}>{signal.severity==='critical'?(ru?'Критично':'Critical'):signal.severity==='high'?(ru?'Высокий':'High'):(ru?'Проверка':'Check')}</span><div><strong>{signal.title}</strong><p>{signal.why}</p>{ref&&<small>{ru?'Владелец':'Owner'}: {ref.owner||(ru?'не указан':'unassigned')}</small>}<p>{signal.action}</p><SignalEvidence signal={signal} locale={locale}/></div><button className="button small" onClick={()=>openSignal(signal)}>{actionLabel(ru,signal.id,Boolean(ref))}</button></li>})}</ol>:<p className="muted">{ru?'Других сигналов в этой группе нет.':'No other signals in this group.'}</p>}{group.items.length>5&&<button className="button small" aria-expanded={expanded.includes(group.id)} onClick={()=>setExpanded(ids=>ids.includes(group.id)?ids.filter(id=>id!==group.id):[...ids,group.id])}>{expanded.includes(group.id)?(ru?'Свернуть':'Show fewer'):(ru?`Показать все (${group.items.length})`:`Show all (${group.items.length})`)}</button>}</section>)}<section className="panel"><h3>{ru?'Стабильно':'Stable'} <span className="muted">{stable.length}</span></h3>{stable.length?<ul>{stable.map(item=><li key={item}>{item}</li>)}</ul>:<p className="muted">{ru?'Недостаточно текущих данных, чтобы подтвердить стабильные области. Отсутствие записи не считается зелёным сигналом.':'There is not enough current data to confirm a stable area. Missing records are not treated as green.'}</p>}</section></div><aside><ProjectHealth workspace={workspace} project={project} locale={locale} onView={onView}/></aside></div>
 </>;
}
