...
export function GuideView({workspace,project,locale,onView,onCreate}:ViewProps) {
 ...
 const charter=()=>{const url=new URL(window.location.href);url.searchParams.set('tab','charter');history.replaceState(null,'',url);dispatchEvent(new Event('pmwork-url'));onView('control')};
 const contours: {name:string;done:boolean;why:string;action:string;run:()=>void}[]=[
  {name:ru?'Цель':'Outcome',done:Boolean(project.objective&&project.successMeasures.length),why:ru?'Без критерия результата нельзя проверить пользу выполненной работы.':'A success measure makes the outcome verifiable.',action:ru?'Уточнить результат':'Define outcome',run:charter},
