"use client";
import { intentRank } from "@/domain/search-intent";
...
export default function PublicSearch({locale,onClose}:{locale:Locale;onClose:()=>void}) {
 ...
 const entries=useMemo(()=>[
  ...
  ...methods.map(t=>({id:`methods-${t.slug}`,label:t.title[locale],search:`${t.title.ru} ${t.title.en} ${t.summary.ru} ${t.summary.en} ${t.bestFit.ru} ${t.bestFit.en}`,group:ru?'МЕТОДЫ':'METHODS',url:`/${locale}/methods/${t.slug}/`})),
 ...
 const intent=entries.filter(e=>intentRank(query,e.id)>0).sort((a,b)=>intentRank(query,b.id)-intentRank(query,a.id));
 const results=[...new Map([...intent,...matching].map(e=>[e.id,e])).values()].slice(0,30);const selected=Math.min(active,Math.max(0,results.length-1));
 ...
 <div className="search-input"><input className="input" type="search" aria-label={ru?'Поиск по PMWORK':'Search PMWORK'} value={query} onChange={e=>{setQuery(e.target.value);setActive(0);}} placeholder={ru?'Опишите проблему, метод, шаблон или термин…':'Describe a problem, method, template or term…'}/><button className="button" onClick={onClose}>{ru?'Закрыть':'Close'}</button></div>
 {query&&intent.length>0&&<p className="muted compact">{ru?'Сначала показаны результаты, связанные с вашей задачей.':'Results related to your intent are ranked first.'}</p>}
