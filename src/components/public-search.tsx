...
export default function PublicSearch({locale,onClose}:{locale:Locale;onClose:()=>void}) {
 ...
 const entries=useMemo(()=>[
  ...
  ...['fit','cpm','pert','evm','monte','rice','wsjf','little'].map(tool=>({id:tool,label:tool.toUpperCase(),search:tool==='evm'?'EVM earned value освоенный объём':tool,group:'TOOLS',url:`/${locale}/tools/?tool=${tool}`})),
 ],[locale,ru]);
 const index=useMemo(()=>new Fuse(entries,{keys:['label','search'],threshold:.35,ignoreLocation:true}),[entries]);
 const results=(query?index.search(query).map(r=>r.item):entries).slice(0,30);const selected=Math.min(active,Math.max(0,results.length-1));
