...
export default function PublicSearch({locale,onClose}:{locale:Locale;onClose:()=>void}) {
 ...
 const entries=useMemo(()=>[
  ...
  ...['fit','cpm','pert','evm','forecast','priority','flow','deadline','emv','capacity','matrix','ownership','change','calibration'].map(tool=>({id:tool,label:tool.toUpperCase(),search:tool==='evm'?'EVM earned value освоенный объём':tool,group:'TOOLS',url:`/${locale}/tools/?tool=${tool}`})),
 ],[locale,ru]);
