...
export function WorkSurface(props: ViewProps) {
  ...
  return <>
    ...
    <div className="toolbar work-toolbar">
      ...
      <button className="button desktop-only" aria-label={ru ? "Добавить работу" : "Add work item"} onClick={() => onCreate("work")}><Plus size={17}/>{ru ? "Добавить работу" : "Add work item"}</button>
    </div>
    ...
    <div className="view-options-row">
      ...
      <details className="view-display"><summary><SlidersHorizontal size={16}/>{ru ? "Вид и сортировка" : "Display and sorting"}</summary><div className="view-config">      <label className="view-control">{ru ? "Статус" : "Status"}<select aria-label={ru ? "Статус" : "Status"} className="input" value={config.status} onChange={e => configure({status:e.target.value as WorkViewConfig["status"]})}><option value="all">{ru ? "Все статусы" : "All statuses"}</option>{statuses.map(s => <option key={s} value={s}>{displayLabel(locale,"workStatus",s)}</option>)}</select></label>
