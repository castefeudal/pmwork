...
export function WorkSurface(props: ViewProps) {
  ...
  const items = selectWork(all, config);
  ...
  const update = (id: string, patch: Partial<WorkItem>) => onChange({...workspace, workItems: workspace.workItems.map(x => x.id === id ? {...x, ...patch, updatedAt: new Date().toISOString(), done: (patch.status ?? x.status) === "done", completedAt: (patch.status ?? x.status) === "done" ? x.completedAt ?? new Date().toISOString() : undefined} : x)});
  ...
  return <>
    ...
    <div className="toolbar work-toolbar">
      ...
      <label className="view-control">{ru ? "Статус" : "Status"}<select className="input" value={config.status} onChange={e => configure({status:e.target.value as WorkViewConfig["status"]})}><option value="all">{ru ? "Все статусы" : "All statuses"}</option>{statuses.map(s => <option key={s} value={s}>{displayLabel(locale,"workStatus",s)}</option>)}</select></label>
      <label className="view-control">{ru ? "Владелец" : "Owner"}<select className="input" value={config.owner} onChange={e => configure({owner:e.target.value})}><option value="">{ru ? "Все владельцы" : "All owners"}</option>{owners.map(s => <option key={s}>{s}</option>)}</select></label>
    ...
    <div className="view-options-row">
      ...
      <details className="view-display"><summary><SlidersHorizontal size={16}/>{ru ? "Вид и сортировка" : "Display and sorting"}</summary><div className="view-config">
        <label className="view-control">{ru ? "Представление" : "Layout"}<select className="input" value={config.type} onChange={e => configure({type:e.target.value as WorkViewConfig["type"]})}><option value="list">{ru ? "Список" : "List"}</option><option value="board">{ru ? "Доска" : "Board"}</option></select></label>
        <label className="view-control">{ru ? "Сортировка" : "Sort"}<select className="input" value={config.sort} onChange={e => configure({sort:e.target.value as WorkViewConfig["sort"]})}>{[["priority","Приоритет","Priority"],["due","Срок","Due date"],["updated","Последнее изменение","Recently updated"],["title","Название","Title"]].map(([id,r,e]) => <option key={id} value={id}>{ru?r:e}</option>)}</select></label>
        <label className="view-control">{ru ? "Группировка списка" : "List grouping"}<select className="input" value={config.group} onChange={e => configure({group:e.target.value as WorkViewConfig["group"]})}>{[["none","Без группировки","None"],["status","Статус","Status"],["owner","Владелец","Owner"],["priority","Приоритет","Priority"]].map(([id,r,e]) => <option key={id} value={id}>{ru?r:e}</option>)}</select></label>
