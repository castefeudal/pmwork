...
for(const locale of ['ru','en'] as const)test(`mobile work keeps records above fold ${locale}`,async({page})=>{
 ...
 await expect(page.getByLabel(ru?'Владелец':'Owner',{exact:true})).toBeHidden();
 ...
 await expect(page.getByLabel(ru?'Владелец':'Owner',{exact:true})).toBeVisible();
...
for(const experience of ['foundation','expert'] as const)test(`advanced fields follow experience ${experience}`,async({page})=>{
 ...
 if(experience==='expert')await expect(field).toBeVisible();
