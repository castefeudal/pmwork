...
test('starter creates one linked bundle and undo survives reload',async({page})=>{
 ...
 await dialog.getByLabel('Type / example (optional)',{exact:true}).selectOption('digital');
...
test('monetary scenario is saved with inputs and survives reload',async({page})=>{
 ...
 await expect(page.getByRole('status')).toContainText('Record saved');await page.reload();
