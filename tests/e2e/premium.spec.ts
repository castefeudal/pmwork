...
test("side editor preserves context and keyboard focus",async({page})=>{
...
test("palette keyboard search opens a record and creates an issue",async({page})=>{
...
test("corrupt browser data is preserved while autosave is paused",async({page})=>{
  ...
  await page.goto(route("/en/workspace/"));await expect(page.getByText("Autosave paused",{exact:true})).toBeVisible();await page.waitForTimeout(700);
...
for(const [width,height] of [[360,800],[390,844],[768,1024],[1024,768],[1280,800],[1440,900],[1920,1080]]){
  test(`workspace reflow ${width}x${height}`,async({page})=>{
...
for(const locale of ["ru","en"] as const) for(const theme of ["light","dark"] as const){
  test(`accessibility representative surfaces ${locale} ${theme}`,async({page})=>{
