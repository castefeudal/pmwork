...
for(const locale of ['ru','en'] as const)for(const width of [360,768,1440])test(`template document is viewport-owned ${locale} ${width}`,async({page},info)=>{
 ...
 await page.goto(route(`/${locale}/templates/`));const trigger=page.getByRole('button',{name:locale==='ru'?'Использовать':'Use template',exact:true}).nth(1);await trigger.click();
