...
const files=[];
function walk(dir) { for (const entry of fs.readdirSync(dir,{withFileTypes:true})) {const file=path.join(dir,entry.name),relative=path.relative('out',file).split(path.sep).join('/');if(entry.isDirectory())walk(file);else if(/\.(html|js|css|woff2|png|webp|svg|webmanifest|txt|xml)$/.test(file)&&relative!=='sw.js'&&!/^(?:404(?:\/|\.)|_not-found\/)/.test(relative))files.push(file);} }
walk("out");
files.sort();
...
const urls=files.filter(file=>!(/^(ru|en)\/glossary\/[^/]+\//.test(exportPath(file)))).map(file=>"./"+exportPath(file).replace(/index\.html$/, ""));
